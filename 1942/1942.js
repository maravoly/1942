...
window.onload = function () {
	...
	if (canvas && canvas.getContext) {
		...
		if (ctx) {
            ...
            // cargar enemigos, animación de ovni y disparos
...
LISTENER
**************/
// mobilización del juego
window.requestAnimationFrame = (function () {
	return window.requestAnimationFrame ||
		window.webkitRequestAnimationFrame ||
		window.mozRequestAnimationFrame ||//activación en los distintos navegadores
		function (callback) { window.setTimeout(callback, 17); }
})();
document.addEventListener("keydown", function (e) {//activación de teclas
	teclaPulsada = e.keyCode;
	tecla[e.keyCode] = true;
});
document.addEventListener("keyup", function (e) {//activación de teclas
...
function nave(x) {
	...
	this.dibuja = function (x) {
		...
		} else if(imgAni2 < 10) {//animación de la nave
...
function Enemigo(x, y) {
	...
	this.dibuja = function () {
		...
		if (this.vive) {
			...
			ctx.fillStyle = "black";//SI UNA NAVE ES ELIMINADA NO APARECE
